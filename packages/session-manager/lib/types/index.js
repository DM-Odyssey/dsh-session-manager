...
let SessionManagerService = (() => {
    ...
    return class SessionManagerService extends _classSuper {
        static {
            ...
            __esDecorate(this, null, _restoreMany_decorators, { kind: "method", name: "restoreMany", static: false, private: false, access: { has: obj => "restoreMany" in obj, get: obj => obj.restoreMany }, metadata: _metadata }, null, _instanceExtraInitializers);
            ...
            __esDecorate(this, null, _deleteAllArchived_decorators, { kind: "method", name: "deleteAllArchived", static: false, private: false, access: { has: obj => "deleteAllArchived" in obj, get: obj => obj.deleteAllArchived }, metadata: _metadata }, null, _instanceExtraInitializers);
        ...
        constructor(ctx) {
            ...
            const tryInit = () => {
                ...
                if (workspaceRegistry === undefined || sessionQuery === undefined || domain === undefined)
                    return;
            ...
            ctx.effect(() => {
                const onAnyCtx = ctx.on.bind(ctx);
                const onChanged = onAnyCtx('domain/changed', (change) => {
                    if (change !== null && typeof change === 'object' && change.domain === 'workspace') {
                        tryInit();
                    }
                });
                let stopPoll;
        ...
        init(ctx, domain, workspaceRegistry, sessionQuery) {
            ...
            ctx.effect(() => {
                ...
                const onAny = ctx.on.bind(ctx);
                onAny('session/disposed', (session) => {
                    const id = session && typeof session.id === 'string' ? session.id : null;
                    if (id === null || !this.pendingDeletes.has(id))
                        return;
                ...
                return () => {
                    if (stopSweep !== undefined)
                        stopSweep();
                    for (const name of ['archiveSession', 'create', 'delete', 'insertBefore']) {
                        const original = originalMethods.get(name);
                        if (original === undefined)
                            workspaceRegistry[name] = undefined;
                        else
                            workspaceRegistry[name] = original;
                    }
                    if (protoDescriptor === undefined) {
                        delete workspaceRegistry.archivedSessionIds;
                    }
                    else {
        ...
        async list() {
            ...
            if (!this.ready)
                return { items: [], groups: [], archivedCount: 0 };
            const state = this.readState();
            ...
            const observations = records.length > 0
                ? await this.sessionQuery.readTitleSnapshots(records.map(record => record.header.id))
                : [];
            ...
            try {
                return await this.enqueue(async () => {
                    if (this.deleted.has(sessionId))
                        return this.fail('session has been deleted');
                    const state = this.readState();
                    if (!state.archivedSessionIds.includes(sessionId)) {
            ...
            try {
                return await this.enqueue(async () => {
                    ...
                    for (const id of targets) {
                        if (this.deleted.has(id)) {
                            failures.push({ id, error: 'session has been deleted' });
                            continue;
                        }
        ...
        async deleteSessions(ids) {
            if (ids.length === 0)
                return this.fail('no session ids provided');
            try {
                return await this.enqueue(async () => {
                    const records = await this.sessionQuery.listSessions();
        ...
        async reconcile() {
            ...
            if (sameIds(next, state.archivedSessionIds))
                return;
            await this.writeState({ ...state, archivedSessionIds: next });
        }
        enqueue(task) {
        ...
        async knownSession(id) {
            ...
            if (live !== undefined && live.get(id) !== undefined)
                return true;
            const records = await this.sessionQuery.listSessions();
        ...
        buildGroups(items) {
            ...
            for (const entry of table.entries()) {
                ...
                for (const sessionId of record.sessionIds) {
                    if (!sessionWorkspace.has(String(sessionId)))
                        sessionWorkspace.set(String(sessionId), wsId);
                }
            }
        ...
        async rmDir(target) {
            if (this.subprocess === null)
                throw new Error('subprocess service is unavailable');
            let handle;
            try {
        ...
        async deleteArtifact(sessionId) {
            ...
            try {
                ...
                if (raw === undefined || raw.meta === undefined)
                    return 'no persisted log artifact found';
                ...
                const target = path !== null
                    ...
                    : null;
                if (target === null)
                    return 'artifact path validation failed; log file not removed';
        ...
        async deleteOne(sessionId, record) {
            ...
            for (const entry of table.entries()) {
                const key = entry[0];
                const value = entry[1];
                if (!value.sessionIds.includes(sessionId))
                    continue;
        ...
        async sweepPending() {
            if (this.pendingDeletes.size === 0 || this.sweeping)
                return;
            this.sweeping = true;
            try {
                const records = await this.sessionQuery.listSessions();
        ...
        async persistState() {
            if (this.statePath === null || this.fsService === null)
                return;
            try {
